...
Definition N_infty :=
  { p : nat -> bool | forall n : nat, p n = true ->
...
Definition S_infty : N_infty -> N_infty.
Proof.
  intros p.
  refine (exist _
                (fun n =>
...
Proof.
  ...
  red. intros.
  assert ((proj1_sig (S_infty x)) (S n) = (proj1_sig (S_infty y)) (S n)).
  { rewrite H. reflexivity. }
  simpl in *.
  assumption.
Qed.
...
Definition N_infty_epsilon : (N_infty -> bool) -> N_infty.
Proof.
  intros Q.
  unshelve refine (exist _ _ _).
  { intros n.
    apply (nat_bounded_forall (fun k => Q (nat_inj k)) n).
  }
  intros.
  simpl in *.
...
(* Theorem 11. Note that there's a typo in the paper in the statement of the theorem. They wrote [p : N_infty -> bool] instead of [p : N_infty].  *)
...
Proof.
  ...
  - right. apply N_infty_epsilon_selection'.
    + intros. apply H in H0. rewrite <- H0. assumption.
...
Proof.
  ...
  - left.
    unfold p in *. destruct H1 as [o].
    destruct (f o); auto. discriminate.
  - right. intros [a].
    specialize (H0 (inl a)) as [o].
    specialize (H1 o). unfold p in *.
...
(* Maybe [N_infty] could be represented by some coinductive stream-structure, and thus create an extensional omniscient type.

Maybe we could use [option nat] and some "calculate" function. i.e. [option nat -> (nat -> bool)].
*)
